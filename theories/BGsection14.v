...
Notation "\ell_ \sigma ( x )" := (sigma_length x)
  (at level 0, format "\ell_ \sigma ( x )") : group_scope.

Notation "''M_' \sigma ( X )" := (sigma_mmax_of X)
  (at level 0, format "''M_' \sigma ( X )") : group_scope.

Notation "''M_' \sigma [ x ]" := (sigma_mmax_of <[x]>)
  (at level 0, format "''M_' \sigma [ x ]") : group_scope.

Notation "''N' [ x ]" := (FT_signalizer_base x)
  (at level 0, format "''N' [ x ]") : group_scope.

Notation "''R' [ x ]" := (FT_signalizer x)
  (at level 0, format "''R' [ x ]") : group_scope.

Notation "M ^~~" := (sigma_cover M)
  (at level 1, format "M ^~~") : group_scope.

Notation "\tau13 ( M )" := (tau13 M)
  (at level 0, format "\tau13 ( M )") : group_scope.

Notation "\kappa ( M )" := (kappa M)
  (at level 0, format "\kappa ( M )") : group_scope.

Notation "\sigma_kappa ( M )" := (sigma_kappa M)
  (at level 0, format "\sigma_kappa ( M )") : group_scope.

Notation "''M_' ''F'" := (TypeF_maxgroups _)
  (at level 0, format "''M_' ''F'") : group_scope.

Notation "''M_' ''P'" := (TypeP_maxgroups _)
  (at level 0, format "''M_' ''P'") : group_scope.

Notation "''M_' ''P1'" := (TypeP1_maxgroups _)
  (at level 0, format "''M_' ''P1'") : group_scope.

Notation "''M_' ''P2'" := (TypeP2_maxgroups _)
  (at level 0, format "''M_' ''P2'") : group_scope.
...
Proof.
move=> p; rewrite unlock {1}inE /= {1}inE /= tau1J tau3J; apply: andb_id2l => _.
...
have [have_a nK1K ntE1 sE1K]: [/\ part_a, b1_hyp, E1 :!=: 1 & E1 \subset K].
  have [t1K | not_t1K] := boolP (\tau1(M).-group K).
    ...
    have st1k: {subset \tau1(M) <= \kappa(M)}.
      move=> p t1p; rewrite unlock !(t1p, inE).
    ...
    have regUK: semiregular U K.
      ...
      rewrite !subG1 /= => /nandP[nregE3Y | nregE2Y].
        ...
        rewrite unlock 2!inE /= (pnatPpi t3E3 piE3q) orbT /=.
...
have regQ: 'C_(M`_\sigma)(Q) = 1.
  ...
  by rewrite unlock inE /= inE /= t1Mq; apply/exists_inP; exists Q.
have nsKM: K <| M.
...
have{t12Hq} [/= t1Hq | /= t2Hq] := orP t12Hq.
  ...
  have kHq: q \in \kappa(H).
    by rewrite unlock inE /= inE /= t1Hq; apply/exists_inP; exists Q.
...
have sK_FD: K \subset 'F(D).
  ...
  case=> [[t2Lq ]|[kLq P1maxL]].
    ...
    by rewrite -defL inE /= inE /= sMst_q in t2Lq.
...
have t2Hq: q \in \tau2(H).
  ...
  case/FtypeP: FmaxH => _ /(_ q)/idP[]; rewrite unlock !(t13Hq, inE).
...
have bNq: q \in \beta('N[x]) by rewrite spM_sbN //=; do 2!rewrite inE /= ?piMq.
...
have regMsQ: 'C_(M`_\sigma)(Q) = 1.
  ...
  by rewrite unlock inE /= inE /= t1Mq; case/exists_inP; exists Q.

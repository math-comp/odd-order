...
Proof.
move=> leWn_q; rewrite !natrM ltf_pV2 ?rpredM ?qualifE ?gt0CG ?ltr0n //.
...
rewrite mulrCA mulKf ?neq0CG // mulrC ler_pM2l ?invr_gt0 ?gt0CG // leC_nat.
...
have ub_e: e%:R <= #|Iirr W|%:R ?= iff (e == p * q)%N :> algC.
  rewrite leif_nat_r card_Iirr_cyclic //; first by have [] := ctiWG.
...
have /(_ predT) := leifD (leif_sum (in1W lb_b)) lbY.
...
rewrite -cfnormDd // -defGa cfnormDd // cfnorm1 -lerBrDr ubGa.
...
have Z1chi: chi \in dirr G.
  ...
  rewrite De (dprod_card xdefW) -card_Iirr_cyclic //; first by have[]:= ctiWG.
...
rewrite cfdotZr cfdot_sumr big1_seq ?mulr0 ?oppr0 => [nu Mnu|].
...
Local Notation "` 'V'" := (gval V) (only parsing) : group_scope.
...
have{ubH} /eqP defH: `H == U.
  ...
  by rewrite -leC_nat natrM -ler_pdivrMr ?gt0CG // {1}(index_sdprod defL).
...
have <-: #|U| = nU.
  ...
  have: nU == 1 %[mod p].
    rewrite /nU predn_exp mulKn; first by rewrite -(subnKC pgt2).
...
rewrite -(@ltn_pmul2l q.-1) ?(mulnCA q.-1); first by rewrite -(subnKC qgt2).
rewrite !mulnA -(@ltn_pmul2l p.-1); first by rewrite -(subnKC pgt2).
...
have nzT1_Ga zeta: zeta \in calT1 -> `|'[Gamma, tau1T zeta]| ^+ 2 >= 1.
  ...
  have QVbetaT0: betaT0 \in 'CF(T, QV^#).
    rewrite cfun_onD1 rpredB ?(seqInd_on _ T1zeta) //=.
...
have lbG0 g: g \in G0 -> 1 <= `|tau1M psi g| ^+ 2.
  ...
  rewrite (bigD1 ij) /=; first by rewrite unfold_in nz_ij eqxx.
...
- rewrite le_gtF // -!addrA -lerBlDl -lerBrDl -(lerD2l 1).
  ...
  rewrite [1 + _ + _]addrA addrAC !natrD !mulrDr !lerD //; first 1 last.
...
rewrite -!addrA ler_ltD //; last first.
  ...
  by rewrite !ltr_pM2l ?ltf_pV2 ?invr_gt0 ?qualifE/= ?gt0CG ?ltr0n ?ltr_nat.
rewrite ler_pdivrMr ?ler_pdivlMl ?gt0CG // -natrM leC_nat.
...
have{ub_v} ub_qp: (q ^ (p - 3) < p ^ 2)%N.
  ...
  have: v.-1%:R < pq ^+ 2.
    rewrite -ltr_pdivrMr ?ltr0n ?muln_gt0 ?cardG_gt0 //.
    by rewrite (le_lt_trans ub_v) // ltrBlDl -mulrS ltC_nat.
...
have ub_h: (h <= p ^ 2 * q ^ 2)%N.
  ...
  rewrite -ltr_pdivrMr ?ltr0n ?muln_gt0 ?cardG_gt0 //.
  by rewrite (le_lt_trans ub_h) // ltrBlDl -mulrS ltC_nat.
...
have /eqP umodpq: u == 1 %[mod p * q].
  ...
  rewrite oU /nU predn_exp mulKn; first by rewrite -(subnKC pgt2).
...
have [[_ ub_h] | [nz_a [q3 p5]]] := LM_cases.
  ...
  rewrite -indexLH -leC_nat natrM -ler_pdivrMr ?gt0CiG // indexLH -/pq.
  by rewrite (le_trans ub_h) // lerBlDl -mulrS leC_nat ltnW.
...
rewrite cfdotBl cfdot_suml big1 => [ij _|].
